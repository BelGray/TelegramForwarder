...
# Telegram ID админа, имеющего доступ к командам в юзер-ботах
ADMIN_ID=123456789
```
...
## Управление через Telegram (Админка)

Вы можете добавлять и удалять каналы прямо через чат с вашим юзерботом (в "Избранном" или в личке с ботом), не заходя на сервер.

**Доступные команды:**
*   `/add_source @ссылка` — Добавить канал-донор (откуда пересылать).
*   `/add_dest @ссылка` — Добавить чат-получатель (куда пересылать).
*   `/list` — Показать список всех подключенных каналов.
*   `/help` — Вывести справку по командам.

**Важно:**
1.  Команды работают только от администратора (ваш ID должен быть указан в настройках).
2.  После добавления нового **источника** (Source) рекомендуется перезагрузить бота, чтобы он начал его слушать. Новые **получатели** (Destinations) подхватываются мгновенно без перезагрузки.
